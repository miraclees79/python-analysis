# -*- coding: utf-8 -*-
"""
TBSP Full Back-extension with Duration, Curvature, Convexity, Intercept, and Volatility Scaling


REJECTED VERSION- EXCESSIVE DRIFT DUE TO NAIVE VOL SCALING
...
# =========================
# 6b. MODEL DIAGNOSTICS
# =========================
corr = np.corrcoef(df_model["ret"], df_model["ret_model"])[0,1]
rmse = np.sqrt(np.mean((df_model["ret"] - df_model["ret_model"])**2))
real_vol = np.std(df_model["ret"])
model_vol = np.std(df_model["ret_model"])
vol_scale = real_vol / model_vol if model_vol > 0 else 1.0

print("\nDiagnostics:")
print(f"Correlation: {corr:.4f}")
print(f"RMSE: {rmse:.6f}")
print(f"Vol real: {real_vol:.6f}, Vol model: {model_vol:.6f}")
print(f"Mean real: {np.mean(df_model['ret']):.6f}, Mean model: {np.mean(df_model['ret_model']):.6f}")
print(f"Volatility scale factor: {vol_scale:.4f}")

# =========================
# 7. FULL SERIES RECONSTRUCTION WITH VOL SCALING
# =========================
...
df_full["ret_model_scaled"] = df_full["ret_model"] * vol_scale
...
# Forward synthetic
df_full.loc[df_full["Date"] >= anchor_date, "TBSP_synth"] = \
    (1 + df_full.loc[df_full["Date"] >= anchor_date, "ret_model_scaled"]).cumprod()
...
for idx in back_idx:
    next_idx = idx + 1
    df_full.loc[idx, "TBSP_synth"] = df_full.loc[next_idx, "TBSP_synth"] / (1 + df_full.loc[idx, "ret_model_scaled"])
...
plt.title("TBSP Index: Real vs Synthetic (Back-extended, Volatility-scaled)")
plt.xlabel("Date")
