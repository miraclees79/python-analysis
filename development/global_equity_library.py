...
def optimise_asset_weights(
    ...
    for combo in combos:
        w = {asset_keys[i]: combo[i] for i in range(n)}
        w_mmf = max(0.0, 1.0 - sum(combo))

        # Daily portfolio return: each asset contributes weight * signal * return
        # Signal=0 → that asset's allocation redirects to MMF
        port_r = mmf_r * w_mmf
        for key in asset_keys:
            sig_day = s[key]
            port_r = port_r + w[key] * (
...
def allocation_walk_forward_n(
    ...
    for _, row in wf_results_ref.iterrows():
        ...
        # ── OOS simulation ────────────────────────────────────────────────
        # OOS signals trimmed to the common OOS period
        def _oos(series):
            return series.loc[
        ...
        for date in oos_sigs[asset_keys[0]].index:
            # Build signal dict for this date
            sigs_today = {k: int(oos_sigs[k].get(date, 0)) for k in asset_keys}
