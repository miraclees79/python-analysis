...
def _extend_series(base_df: pd.DataFrame, ext_df: pd.DataFrame) -> pd.DataFrame:
    ...
    base_df.index = (
        base_df.index.tz_localize(None) if base_df.index.tz is not None else base_df.index
    )
    ...
    first_ret = (
        new_data[CLOSE_COL].iloc[0]
        / ext_df[CLOSE_COL].reindex([anchor_date], method="ffill").iloc[0]
    ) - 1
...
def _build_full_msci_world(
    client: GDriveClient, folder_id: str, wsj_combined_df: pd.DataFrame,
) -> pd.DataFrame:
...
def build_and_upload(
    ...
    if existing is not None:
        ...
        logging.info(
            f"Loaded existing {combined_filename} from Drive (Starts: {base_df.index.min().date()})",
        )
    ...
    if base_df is None:
        if is_msci_world:
            ...
            base_df = _build_full_msci_world(
                client=client, folder_id=folder_id, wsj_combined_df=None,
            )
    ...
    if extension_source == "yfinance":
        ...
        try:
            ext_data = yf.download(
                tickers=extension_ticker, start=start_dt, progress=False, auto_adjust=True,
            )
            ...
            logging.warning(f"Extension Error ({extension_ticker}): {e}")
    ...
    if is_msci_world:
        base_df = _build_full_msci_world(
            client=client, folder_id=folder_id, wsj_combined_df=base_df,
        )
