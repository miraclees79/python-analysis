...
def load_local_csv(ticker: str, label: str, data_start: str = "1990-01-01", mandatory: bool = True) -> pd.DataFrame | None:
    """
    ...
    try:
        ...
        df = pd.read_csv(path, on_bad_lines='skip', delimiter=',', decimal='.', encoding='utf-8-sig')
    except Exception as e:
        logging.error(f"Error reading {path}: {e}")
        return None

    if df.empty or 'Data' not in df.columns:
        if mandatory: logging.error(f"Corrupted or empty file: {path}")
        return None
    ...
    logging.info(f"OK  : {label:<14} {len(df):>5} rows  {df.index.min().date()} to {df.index.max().date()}")
    return df