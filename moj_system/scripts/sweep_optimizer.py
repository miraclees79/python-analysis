...
class SweepManager:
    ...
    def get_cached_wf(
        ...
        if cache_key in self.wf_cache:
            logging.info(
                f"  [WF CACHE HIT] {asset_name} train: {train_y} test: {test_y}, stop type: {stop_type}",
            )
    ...
    def get_cached_mc(
        ...
        if cache_key in self.mc_cache:
            logging.info(
                f"  [MC CACHE HIT] {asset_name} train: {train_y} test: {test_y}, stop type: {stop_type}",
            )
    ...
    def get_cached_boot(
        ...
        if cache_key in self.boot_cache:
            logging.info(
                f"  [BOOT CACHE HIT] {asset_name} train: {train_y} test: {test_y}, stop type: {stop_type}",
            )
    ...
    def _prepare_pension_data(self):
        return build_standard_two_asset_data(
            ...
            mmf=self.data_map["MMF_EXT"],
            wibor1m=self.data_map.get("WIBOR1M"),
            pl10y=self.data_map["PL10Y"],
            de10y=self.data_map["DE10Y"],
    ...
    def _compile_full_result(
        ...
        window_rows = _extract_window_rows(
            asset_name=strat_name,
            train_years=train_y,
            test_years=test_y,
            wf_results=wf_results,
            stop_mode=stop_type,
            alloc_df=alloc_df,
            weights_series=weights_series,  # <--- DODANO
        )
        self.all_windows.extend(window_rows)
        win_stats = _compile_window_stats(window_rows)

        mc_verdict = (
            "|".join([f"{k[:3]}:{v['verdict'][:3]}" for k, v in mc_verdicts_dict.items()])
            if mc_verdicts_dict
            else "N/A"
        )
        bb_verdict = (
            "|".join([f"{k[:3]}:{v['verdict'][:3]}" for k, v in bb_verdicts_dict.items()])
            if bb_verdicts_dict
            else "N/A"
        )
        ...
        return {
            ...
            "test_years": test_y,
            "stop_mode": stop_type,
            "oos_cagr": m_trimmed.get("CAGR", pd.NA),
            "oos_calmar": m_trimmed.get("CalMAR", pd.NA),
            "oos_maxdd": m_trimmed.get("MaxDD", pd.NA),
            "oos_sharpe": m_trimmed.get("Sharpe", pd.NA),
            "oos_sortino": m_trimmed.get("Sortino", pd.NA),
            "bh_cagr": bh_metrics.get("CAGR", pd.NA),
            "bh_calmar": bh_metrics.get("CalMAR", pd.NA),
            "bh_maxdd": bh_metrics.get("MaxDD", pd.NA),
            "bh_sharpe": bh_metrics.get("Sharpe", pd.NA),
            "cagr_vs_bh": (m_trimmed.get("CAGR", 0) - bh_metrics.get("CAGR", 0)) * 100
            if pd.notna(m_trimmed.get("CAGR")) and pd.notna(bh_metrics.get("CAGR"))
            else pd.NA,
    ...
    def run_pension_iteration(self, train_y, test_y, stop_type_eq, common_start):
        ...
        if self.n_mc > 0:
            ...
            mc_res["TBSP"] = self.get_cached_mc(
                asset_name="TBSP",
        ...
        bb_res = {}
        if self.n_boot > 0:
            bb_res["WIG"] = self.get_cached_boot(
                asset_name="WIG",
                df=WIG,
                cash_df=MMF_EXT,
                n_samples=self.n_boot,
            ...
            bb_res["TBSP"] = self.get_cached_boot(
                asset_name="TBSP",
                df=TBSP,
                cash_df=MMF_EXT,
                n_samples=self.n_boot,
        ...
        # 4. Allocation OOS
        sig_eq, sig_bd = (
            build_signal_series(wf_equity=wf_eq, wf_trades=wf_tr_eq),
            build_signal_series(wf_equity=wf_bd, wf_trades=wf_tr_bd),
        )
        ...
        return self._compile_full_result(
            ...
            mc_verdicts_dict=mc_res,
            bb_verdicts_dict=bb_res,
            alloc_df=alloc_df,
    ...
    def run_global_iteration(self, variant_key, train_y, test_y, stop_type_eq, common_start):
        ...
        for lbl, (px_df, fx_s) in assets.items():
            ...
            if self.n_boot > 0:
                bb_res[lbl] = self.get_cached_boot(
                    asset_name=lbl,
                    df=proc_px,
                    cash_df=MMF_EXT,
                    n_samples=self.n_boot,
        ...
        # TBSP (Shared Gated)
        wf_bd_e, wf_bd_r, wf_bd_t = self.get_cached_wf(
            asset_name="TBSP",
            df=TBSP,
            train_y=train_y,
            test_y=test_y,
        ...
        port_eq, port_wgts, realloc_log, alloc_df = allocation_walk_forward_n(
            ...
            sigs_full,
            sigs_full,
            MMF_EXT["Zamkniecie"].pct_change().dropna(),
            wf_bd_r,
            list(rets_dict.keys()),
            train_years=train_y,
        )
...
def print_sweep_report(results_df: pd.DataFrame, common_start):
    ...
    if regime_cols:
        ...
        key_regime_cols = [
            ...
            "test_years",
            "stop_mode",
            "adx_uptrend_strat_cagr",
            "adx_uptrend_bh_cagr",
            "adx_downtrend_strat_cagr",
            "adx_downtrend_bh_cagr",
...
def main():
    ...
    data_map["TBSP"] = build_and_upload(
        folder_id,
    ...
    data_map["MSCI_WORLD"] = build_and_upload(
        folder_id,
