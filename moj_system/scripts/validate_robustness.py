...
class ValidationManager:
    ...
    def validate_single(self, asset_name, train_y, test_y, stop_type, df, cash_df):
        ...
        if self.n_mc > 0:
            ...
            analyze_robustness(
                ...
                baseline_metrics=compute_metrics(wf_eq),
    ...
    def validate_pension(self, train_y, test_y, stop_type_eq):
        ...
        if self.n_mc > 0:
            ...
            analyze_robustness(
                results_df=mc_bd,
                baseline_metrics=compute_metrics(equity=wf_bd_eq),
    ...
    def validate_global(self, variant, train_y, test_y, stop_type_eq):
        ...
        if self.n_mc > 0:
            ...
            analyze_robustness(
                results_df=mc_bd,
                baseline_metrics=compute_metrics(equity=wf_bd),
                thresholds=BOND_THRESHOLDS_MC,
            )
...
def main():
    ...
    parser.add_argument(
        "--weights_perturb",
        action="store_true",
        help="Uruchom dodatkowy test perturbacji wag alokacji.",
    )
