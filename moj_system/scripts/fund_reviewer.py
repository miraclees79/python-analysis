...
from moj_system.core.fund_analytics import FundPerformanceEngine, BenchmarkComparator
...
FINAL_COLS = [
    "subfundId", "stooq_id", "knf_name", "tfi_name", "category", "classification", 
    "riskLevel", "managementAndOtherFeesRate", "maxEntryFeeRate", "hasBenchmark", 
    "benchmark", "window", "ann_return", "ann_vol", "sharpe", "sortino", 
    "max_drawdown", "calmar", "data_years", "n_days", "insufficient", "fetch_ok",
    "alpha", "beta", "tracking_error", "ir", "pct_ir_positive", "n_ir_windows", 
    "category_funds", "category_peers_ok", "rank_ir", "rank_sharpe", 
    "rank_sortino", "rank_calmar", "rank_return"
]
...
class FundReviewer:
    ...
    def load_confirmed_funds(self):
        ...
        if not self.folder_id: sys.exit("GDRIVE_FOLDER_ID not set.")
        
        df_conf = self.gdrive.download_csv(self.folder_id, "knf_stooq_confirmed.csv", sep=",")
        if df_conf is None: sys.exit("Could not load confirmed list.")
    ...
    def evaluate_fund(self, fund_row, prices, cat_benchmarks):
        ...
        for win_name, win_days in WINDOWS.items():
            ...
            if abs_m["insufficient"]: continue
            ...
            if win_name == "all":
                for b_name, b_prices in self.benchmarks.items():
                    if b_prices is not None:
                        win_rates, _, _ = BenchmarkComparator.compare_fund_to_benchmark(prices, b_prices)
    ...
    def generate_reports(self):
        ...
        for metric in ["ir", "sharpe", "sortino", "calmar", "ann_return"]:
            ...
            if metric in rank_df.columns:
                rank_df[rank_col] = rank_df.groupby(["category", "window"])[metric].rank(method="min", ascending=False)
            else:
                rank_df[rank_col] = np.nan
        
        # Enforce exact columns and order
        for col in FINAL_COLS:
            if col not in rank_df.columns: rank_df[col] = np.nan
        ...
        perf_path, rank_path = f"outputs/fund_performance_{today}.csv", f"outputs/fund_ranking_{today}.csv"
